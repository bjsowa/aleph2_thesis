...
\begin{itemize}
    \item \textbf{Unstable implementation} -\todomma{zamiast znaku półpauzy należy użyć znak dywizu --} the software responsible for the drivetrain often fails to communicate with the motor drivers due to only\todomma{słowo only zbędne} partial implementation of the communication protocol and various race conditions.
...
This section describes some of the main concepts of technologies the software is based on, which are essential in order to understand certain parts of this thesis.
    ...
    \begin{itemize}
        \item \textbf{Computation graph}\todomma{może użyć środowiska description zamiast itemize?}\\
