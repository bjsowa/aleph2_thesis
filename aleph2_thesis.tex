...
\item Put the packages you want to build in the source space. To launch the simulation, only \textsf{aleph2\_description} and \textsf{aleph2\_gazebo} are required. If you want to build the whole package stack, you will need to download additional packages mentioned in \ref{overview}:
...
\item (Optionally) Add the line above to \textsf{.bashrc} file to source the result space automatically at the start of every terminal session:
...
To send position commands to the arm joint, you need to stop the effort controller and start the position controller first (they cannot run at the same time). In order to do this, you need to make a call to the \textsf{switch\_controller} service, for example by using the \textsf{rosservice} tool: 
