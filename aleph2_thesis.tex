...
The instructions were tested on \textsf{Ubuntu 18.04} system which is the main supported platform by the latest ROS distribution (at the time this thesis was written). The process of installation may be slightly different for other distributions.

\section{Installation}
Before building the packages, the following prerequisites need to be installed:
...
\lstinputlisting[language=bash, firstline=1, lastline=2]{lst/installation.lst}

\section{Launching}

\chapter{Conclusion}
It is fair to say that the new control software for the \rovername rover meets the goals described in section \ref{goals}. It provides a great base for students to develop the rover further. 
...
\begin{itemize}
	...
	\item \textbf{Modeling the kinematic chain of the suspension} -- adding position sensors to the rover's suspension mechanism; modyfing the URDF model so that the exact state of the rockers and differential bar can be visualized by the sensors readings; simulating the rocker suspension in Gazebo by modeling a closed-loop kinematic chain.
\end{itemize}
