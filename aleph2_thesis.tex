...
\pagebreak
    ...
    \begin{itemize}
        \item \textbf{rubi\_server} -- a ROS node that keeps track of connected devices (reffered in RUBI as \textit{boards}) and exposes their capabilities to ROS topics. It communicates with the boards using a custom protocol over CAN (Common Area Network) bus. 
...
\section{Overview of provided packages}
    ...
    \begin{itemize}
        \item \href{https://gitlab.continuum.ii.uni.wroc.pl/continuum/software/kacanopen}{\textbf{kacapopen}} -- 
        provides a set of libraries for communication with devices that implement the CANopen protocol (\cite{kacanopen2016}). The package was adapted by the \textit{Continuum} team to follow a file structure that is more consistent with the ROS ecosystem. 
        \item \href{https://gitlab.continuum.ii.uni.wroc.pl/continuum/software/nanotec_driver}{\textbf{nanotec\_driver}} -- 
        contains a C++ library that utilizes \textsf{kacanopen} to provide an interface for communication with \textit{Nanotec} motor drivers which are used extensively on the \rovername robot.
        \item \href{https://github.com/acriaer/rubi_server}{\textbf{rubi\_server}} -- 
        contains the server node for communication with RUBI-enabled devices and message definitions for the ROS side of the protocol.
    \end{itemize}
...
\section{aleph2\_description}
    ...
    The purpose of this package is to provide an URDF model of the \rovername robot that describes its visual, kinematic and physical properties. It uses \href{http://wiki.ros.org/xacro}{\textsf{xacro}} to make the description more human-readable, by using macros, math expressions, conditional blocks and include mechanics. 
    ...
    \dirtree{%
        .1 aleph2\_description/.
            ...
            .2 rviz/ 
            \filedesc{configuration files for RViz}.
            .2 CMakeLists.txt.
            .2 package.xml.
    }
